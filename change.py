    ...
    try:
        ...
        if keyboard.is_pressed("alt+m"):  # "caps lock" キーが押されたとき
            if is_muted:
                ...
                threading.Thread(target=show_notification,args=("みゅーとくん","マイクミュートを解除しました",rf"{thispath}\icon.png"),daemon=True).start()
            else:
                volume.SetMasterVolumeLevelScalar(0.0, None)  # ミュート
                threading.Thread(target=show_notification,args=("みゅーとくん","マイクをミュート状態にしました",rf"{thispath}\icon.png"),daemon=True).start()
